...
from ..sync.app_sync_manager import AppSyncManager
from ..sync.app_sync_models import AppSyncProvider, AppSyncConfig, ConflictStrategy, SyncDirection
from ..credential_manager import CredentialManager
from ..sync.sync_mapping_store import SyncMappingStore
from ..sync.providers import TodoistAdapter
...
def get_app_sync_manager() -> AppSyncManager:
    ...
    from ..sync.app_sync_config import get_app_sync_config_manager
    ...
    for provider in config_manager.get_all_providers():
        try:
            ...
            if provider == AppSyncProvider.TODOIST:
                from ..sync.providers.todoist_adapter import TodoistAdapter
...
async def _setup_todoist(cred_manager: CredentialManager, interactive: bool, api_token: Optional[str],
    ...
    try:
        ...
        try:
            from ..sync.providers.todoist_adapter import TodoistAPI
...
async def _setup_apple_reminders(cred_manager: CredentialManager, interactive: bool, 
    ...
    try:
        from ..sync.providers.apple_reminders_adapter import AppleRemindersAdapter
        from ..sync.app_sync_models import AppSyncConfig, AppSyncProvider, SyncDirection, ConflictStrategy
...
async def _save_sync_config(config: AppSyncConfig):
    ...
    from ..sync.app_sync_config import get_app_sync_config_manager, ProviderSettings
