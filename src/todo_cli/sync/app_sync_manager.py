...
class _AsyncMappingStoreProxy:
    """Wrap mapping store to accept sync or async implementations."""
...
class AppSyncManager:
    ...
    def __init__(self, storage: Storage):
        ...
        self._mapping_store_wrapper = None
    ...
    def mapping_store(self):
        ...
        if (self._mapping_store_wrapper is None or
                getattr(self._mapping_store_wrapper, "target", None) is not self._mapping_store):
            self._mapping_store_wrapper = _AsyncMappingStoreProxy(self._mapping_store)
        return self._mapping_store_wrapper
    ...
    def register_adapter(self, provider: AppSyncProvider, adapter: AppSyncAdapter):
        ...
        if isinstance(config, AppSyncConfig):
            ...
            resolved_config = AppSyncConfig(provider=provider)
            setattr(adapter, "config", resolved_config)

        self.adapters[provider] = adapter
        self.configs[provider] = resolved_config
            ...
            try:
                ...
                if existing_mapping:
                    ...
                    if (existing_mapping.local_hash and 
                        ...
                        # Ensure remote item still exists before attempting update
                        remote_exists = True
                        if hasattr(adapter, 'verify_item_exists'):
                            remote_exists = await adapter.verify_item_exists(existing_mapping.external_id)
                        if not remote_exists:
                            await self._handle_deletion_conflict(existing_mapping, todo, None, result)
                            continue
            ...
            if todo_id not in seen_todo_ids:
                ...
                try:
                    ...
                    if exists:
                        ...
                        self.logger.info(
                            f"Remote task {mapping.external_id} already deleted; cleaned up mapping"
                        )
    ...
    async def _handle_deletion_conflict(self, mapping: SyncMapping, local_todo: Optional[Todo], 
        ...
        elif not local_todo and not remote_item:
            conflict_type = ConflictType.DELETED_LOCAL
        else:
            conflict_type = ConflictType.MODIFIED_BOTH
        ...
        both_deleted = not local_todo and not remote_item
        ...
        if both_deleted:
            ...
            result.conflicts_detected += 1
            self.logger.warning(
                f"Deletion conflict detected: {conflict_type.value} for todo {mapping.todo_id}"
            )
