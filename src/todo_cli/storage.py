...
class TodoMarkdownFormat:
    ...
    @staticmethod
    def from_markdown(line: str, project: str = "inbox", line_id: Optional[int] = None) -> Optional[Todo]:
        ...
        todo_id = int(id_match.group(1)) if id_match else (line_id or 1)
        line = re.sub(r"\\s*<!-- id:\\d+ -->", "", line)
        ...
        patterns = [
            (r"@(\\w+)", lambda m: tags.append(m.group(1)) if not any(ctx in m.group(1) for ctx in ["home", "work", "phone", "office"]) else context.append(m.group(1))),
            (r"\\^(\\d{4}-\\d{2}-\\d{2})", lambda m: setattr(locals(), 'start_date', datetime.strptime(m.group(1), '%Y-%m-%d'))),
            (r"!(\\d{4}-\\d{2}-\\d{2})", lambda m: setattr(locals(), 'due_date', datetime.strptime(m.group(1), '%Y-%m-%d'))),
            (r"~(critical|high|medium|low)", lambda m: setattr(locals(), 'priority', Priority(m.group(1)))),
            (r"\\*(\\w+)", lambda m: setattr(locals(), 'effort', m.group(1))),
            (r"\\+(\\w+)", lambda m: assignees.append(m.group(1))),
            (r"&(\\w+)", lambda m: stakeholders.append(m.group(1))),
            (r"%(\\w+:?\\w*)", lambda m: setattr(locals(), 'recurrence', m.group(1))),
            (r"\\[PINNED\\]", lambda m: setattr(locals(), 'pinned', True)),
            (r"\\(waiting: ([^)]+)\\)", lambda m: waiting_for.extend([w.strip() for w in m.group(1).split(",")])),
        ]
...
class Storage:
    ...
    def backup_project(self, project_name: str, backup_path: Optional[Path] = None) -> bool:
